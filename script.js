document.addEventListener('DOMContentLoaded', function () {
  ...
  document.body.addEventListener('submit', function (e) {
    ...
    fields.forEach(function (field) {
      ...
      } else if (field.minLength && field.minLength > 0 && value !== '' && value.length < field.minLength) {
  ...
  function getFieldLabel(field) {
    ...
    if (placeholder) return placeholder.replace('*', '').trim();
  ...
  animationStyle.textContent = [
    ...
    '}',
    '.fade-in-up-init.is-visible {',
    '  opacity: 1;',
    '  transform: translateY(0);',
    '}',
    '.fade-in-up-init:nth-child(2) { transition-delay: 0.1s; }',
    '.fade-in-up-init:nth-child(3) { transition-delay: 0.2s; }',
    '.fade-in-up-init:nth-child(4) { transition-delay: 0.3s; }',
    '.fade-in-up-init:nth-child(5) { transition-delay: 0.4s; }',
    '.fade-in-up-init:nth-child(6) { transition-delay: 0.5s; }',
    '.form-error-message {',
    '  display: block;',
    '  color: #dc3545;',
    '  font-size: 0.85em;',
  ...
  const combinedSelector = ANIMATION_SELECTORS.join(', ');
  let animatableElements = [];

  try {
    animatableElements = Array.from(document.querySelectorAll(combinedSelector));
  } catch (e) {
    animatableElements = [];
  }
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "taktokkids משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
    ...
    (function() {
        ...
        function initContactForm() {
            ...
            try {
                ...
                const payload = {
                    websiteId: '6b8d419a-8a26-4a65-ace0-37fadb36703c',
                    name: name,
                    email: email,
                    subject: subject,
                    message: message,
                ...
                if (result.success) {
                    ...
                    if (result.thankYouPagePath && result.ticketNumber) {
                        ...
                        if (isPreviewMode && window.ZAPPY_CONFIG) {
                            ...
                            const websiteId = window.ZAPPY_CONFIG.websiteId || '6b8d419a-8a26-4a65-ace0-37fadb36703c';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '6b8d419a-8a26-4a65-ace0-37fadb36703c';
